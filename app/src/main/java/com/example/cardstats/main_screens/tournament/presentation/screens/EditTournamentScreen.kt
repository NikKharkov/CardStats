...
fun EditTournamentScreen(
    ...
    LaunchedEffect(cardGames) {
        if (cardGames.isNotEmpty()) {
            tournamentViewModel.getTournamentById(tournamentId)?.let { tournament ->
                tournamentName = tournament.tournamentName

                val gameIndex = tournament.cardGameId - 1
                if (gameIndex in cardGames.indices) {
                    addGameViewModel.updateSelectedGame(cardGames[gameIndex])
                }

                val dateFormat = SimpleDateFormat("dd.MM.yyyy", Locale.getDefault())
                try {
                    val startDate = dateFormat.parse(tournament.startDate)
                    val endDate = dateFormat.parse(tournament.endDate)
                    if (startDate != null) {
                        val calendar = Calendar.getInstance().apply { time = startDate }
                        tournamentViewModel.updateFromDate(
                            LocalDate(
                    ...
                    if (endDate != null) {
                        ...
                        tournamentViewModel.updateToDate(LocalDate(
                            calendar.get(Calendar.YEAR),
                            calendar.get(Calendar.MONTH) + 1,
                            calendar.get(Calendar.DAY_OF_MONTH)
                        ))
                    ...
                    Log.e("EditTournamentScreen", "Invalid date format: ${e.message}")
                }

                val mainParticipants = tournamentViewModel.getParticipants(tournamentId).firstOrNull()
                mainParticipants?.firstOrNull()?.let { participant ->
                    points = participant.points.toString()
                    wins = participant.wins.toString()

                    val otherParticipants = tournamentViewModel.getOtherParticipants(participant.id)
                        .firstOrNull() ?: emptyList()
                    tournamentViewModel.setParticipants(
                        otherParticipants.map {
                            TempParticipant(it.name, it.points.toString(), it.wins.toString())
                        }
                    )
                }
            }
                    ...
                    NavigationButton(
                        ...
                        onClick = {
                            scope.launch {
                                ...
                                tournamentViewModel.updateTournamentAndParticipants(
                                    tournamentId = tournamentId,
                                    tournamentName = tournamentName,
                                    cardGameId = cardGameId,
                                    startDate = formattedFromDate,
                                    endDate = formattedToDate,
                                    playerPoints = points.toInt(),
                                    playerWins = wins.toInt()
                                )
